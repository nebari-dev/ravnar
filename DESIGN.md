...
| `output.structuredOutput`  | `agent._output_schema`          | If `_output_schema` is `None`, leave as `None` (unset). If it is a `TextOutputSchema`, leave as `None` (plain text output). If it is a `JsonOutputSchema` or `ObjectOutputSchema`, set to `True`. |
...
- Import `ag_ui.core.Tool` and `ag_ui.core.SubAgentInfo` for building tool lists and sub-agent info. The AG-UI types (`AgentCapabilities`, `Tool`, `SubAgentInfo`, `IdentityCapabilities`, etc.) come from the `ag-ui-protocol` package, available under the `ag_ui.core` import.
