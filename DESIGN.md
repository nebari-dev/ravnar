...
| `identity.metadata`        | `agent.metadata`                | Direct attribute read if set (type is `AgentMetadata \| None`). If `None`, omitted.                                                                                                                                                                                                                                                       |
...
| `tools.items`              | `agent._function_toolset.tools` | Iterate the `dict[str, Tool]` values. Each `Tool` has `.name`, `.description`, and `.function_schema.json_schema` which maps to AG-UI `Tool.parameters`.                                                                                                                                                                                  |
| `tools.clientProvided`     | Always `True`                   | Hardcoded — Pydantic AI always accepts runtime tools.                                                                                                                                                                                                                                                                                     |
| `output.structuredOutput`  | `agent._output_schema`          | True if `_output_schema` is not a `TextOutputSchema` (i.e., a `JsonOutputSchema` or `ObjectOutputSchema`). The `TextOutputSchema` case means plain text output.                                                                                                                                                                           |
| `transport.streaming`      | `True`                          | Hardcoded — Pydantic AI supports streaming via `Agent.run_stream()`.                                                                                                                                                                                                                                                                      |
| `reasoning.supported`      | `agent.capabilities`            | Iterate `agent.capabilities`. If any item is an instance of `pydantic_ai.capabilities.Thinking`, set to `True`. Factory callables (e.g., `Callable[[RunContext], ...]`) are silently skipped. If no `Thinking` capability is found, leave as `None` (unset) — absence of the capability does not prove the agent lacks reasoning support. |
...
| `humanInTheLoop.approvals` | `agent.capabilities`            | Iterate `agent.capabilities`. If any item is an instance of `HandleDeferredToolCalls`, set to `True`. Factory callables are silently skipped. If not found, leave as `None`. Note: deferred tools can mean either human approval or external execution; this mapping is conservative.                                                     |
| `multimodal.output.image`  | `agent.capabilities`            | Iterate `agent.capabilities`. If any item is an instance of `ImageGeneration`, set to `True`. Factory callables are silently skipped. If not found, leave as `None`.                                                                                                                                                                      |

#### Tool extraction detail (Pydantic AI)

Tools live in `agent._function_toolset.tools` as a `dict[str, Tool]`. Each `Tool` provides:
...
| `reasoning.supported`      | `agent.reasoning`                                               | Direct boolean attribute. If the attribute is unset or `None`, leave as `None` (do not set `False`).                                                                                                                                                                                                                                                                                                                  |
...
- Import `ag_ui.core.Tool` and `ag_ui.core.SubAgentInfo` for building tool lists and sub-agent info.
